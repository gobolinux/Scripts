	...
	if name:
		if not isList:
			...
			try:
				...
				for file in files:
					# The check below ought to take place, but the os.environ call
					# above is failing to read array environment variables, so
					# it needs to go through bash at least once when the variable
					# already exists, even if the file is invalid.
					#
					# if os.path.isfile(file):
		...
		for file in files:
			...
			if os.path.isfile(file):
				with open(file, "r") as f:
					ret = f.readlines()
			if ret:
				return [ line.split('#')[0].strip() for line in ret if line.split('#')[0].strip() ]
		return []
...
def _bash2(command, mode='o'):
	...
	os.write(stdin_w, str.encode(command + '\nexit $?\n'))
	os.close(stdin_w)
	output = []
	ret = p.wait()
	os.close(stdin_r)
	if stdout_r:
		os.close(stdout_w)
		out = os.read(stdout_r, 32768).decode("utf-8")
		while out != '':
			output.append(out)
			out = os.read(stdout_r, 32768).decode("utf-8")
...
def getCompileOptions():
	...
	def getVariable(varName):
		value = None
		try:
			value = os.environ[varName].strip('\n').split('\n')
		except KeyError:
			for fname in compileSettingsFiles:
				cacheKey = fname + " -> " + varName
				if cacheKey in compileOptionsCache:
					return compileOptionsCache[cacheKey]
				if os.path.isfile(fname):
					cmd = ". \"{}\" 2> /dev/null;".format(fname)
					cmd += "for i in \"${{{}[@]}}\"; do echo \"$i\"; done".format(varName)
					value = bash(cmd).split('\n')
				if value and value[0]:
					compileOptionsCache[cacheKey] = value
					break
		return value

	compileRecipeDirs = getVariable('compileRecipeDirs')
	getRecipeStores = getVariable('getRecipeStores')
	...
	import os.path
	return (list(map(os.path.expanduser,compileRecipeDirs)), getRecipeStores)
...
class KeyInsensitiveDict:
	...
	def __iter__(self):
		return iter((v[0] for v in list(self._dict.values())))
