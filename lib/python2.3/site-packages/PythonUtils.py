...
def Log(message, scriptName, color):
	...
	for line in message.split('\n'):
		sys.stderr.write(colorGray+persistentScriptName+':'+colorNormal+" "+color+line+colorNormal+"\n")
...
def getGoboVariable(name, filename = 'GoboPath', isList = False):
	...
	if filename != 'GoboPath':
		...
		files = [ goboUserSettings+"/"+filename, goboSettings + "/"+filename, goboScriptsDefaults + "/"+filename, goboManagerDefaults+"/"+filename, filename]
	else:
		files = [ filename]
	if name:
		if not isList:
			try:
				...
				for file in files:
					value = os.popen("bash -c '. \"%s\" 2> /dev/null; echo -n \"$%s\"'"%(file,name)).read().strip('\n')
					if value:
						break
			...
			try:
				...
				for file in files:
					os.environ[name] = os.popen("bash -c '. \"" + file +"\" 2> /dev/null; for i in \"${"+name+"[@]}\"; do echo \"$i\"; done'").read().strip('\n')
					if os.environ[name]:
						break
				return os.environ[name].split()
	else:
		...
		for file in files:
			ret = os.popen("bash -c 'cat \"" + file +"\" 2> /dev/null'").read().strip('\n')
			if ret:
				return [ line.split('#')[0].strip() for line in ret.split('\n') if line.split('#')[0].strip() ]
		return []

def bash(command, output=True, value=False):
	"""Execute a bash command. Return the output, the return value, or both."""
	(stdin, stdout) = os.popen2('/bin/bash', 'rw')
	stdin.write(command)
	ret = stdin.close()
...
def getCompileOptions():
	...
	try:
		...
		for file in compileSettingsFiles:
			compileRecipeDirs = os.popen("bash -c '. \"" +file+"\" 2> /dev/null; for i in \"${compileRecipeDirs[@]}\"; do echo \"$i\"; done'").read().strip('\n').split('\n')
	...
	try:
		...
		for file in compileSettingsFiles:
			getRecipeStores = os.popen("bash -c '. \"" + file +"\" 2> /dev/null; for i in \"${getRecipeStores[@]}\"; do echo \"$i\"; done'").read().strip('\n').split('\n')
